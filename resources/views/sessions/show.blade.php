...
<div class="presenter-shell {{ $displayMode ? 'display-mode' : '' }}" data-index="{{ $activeIndex }}">
    ...
    <footer class="presenter-console">
        <div class="console-nav"><button id="prev" title="Diapositiva anterior">←</button><b id="position">{{ $activeIndex + 1 }}</b><span>/ {{ $slides->count() }}</span><button id="next" title="Diapositiva siguiente">→</button></div>
        <div class="question-nav"><button id="question-prev" title="Interacción anterior">‹</button><span id="question-position">Mensaje</span><button id="question-next" title="Siguiente interacción">›</button></div>
...
<script>
(()=>{const shell=document.querySelector('.presenter-shell'),slides=[...document.querySelectorAll('.stage-slide')],pos=document.querySelector('#position'),qPos=document.querySelector('#question-position'),connected=document.querySelector('#connected-count');let index=Number(shell.dataset.index)||0,activityIndex=-1,ticks=0;const csrf=document.querySelector('meta[name="csrf-token"]').content;
const activities=()=>[...slides[index].querySelectorAll('.stage-question')];const paintActivity=()=>{const list=activities();list.forEach((q,i)=>q.classList.toggle('is-active',i===activityIndex));qPos.textContent=activityIndex<0?'Mensaje':`Pregunta ${activityIndex+1}/${list.length}`};
const show=async(next,sync=true)=>{index=Math.max(0,Math.min(slides.length-1,next));activityIndex=-1;slides.forEach((s,i)=>s.classList.toggle('is-active',i===index));pos.textContent=index+1;paintActivity();if(sync)await fetch('{{ route('sessions.slide',$session) }}',{method:'PUT',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':csrf,'Accept':'application/json'},body:JSON.stringify({slide_id:slides[index].dataset.slideId})});};
const showActivity=async next=>{const list=activities();activityIndex=Math.max(-1,Math.min(list.length-1,next));paintActivity();await fetch('{{ route('sessions.activity',$session) }}',{method:'PUT',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':csrf,'Accept':'application/json'},body:JSON.stringify({activity_id:activityIndex<0?null:Number(list[activityIndex].dataset.activityId)})})};
const initial=activities().findIndex(q=>q.classList.contains('is-active'));activityIndex=initial;paintActivity();document.querySelector('#prev')?.addEventListener('click',()=>show(index-1));document.querySelector('#next')?.addEventListener('click',()=>show(index+1));document.querySelector('#question-prev')?.addEventListener('click',()=>showActivity(activityIndex-1));document.querySelector('#question-next')?.addEventListener('click',()=>showActivity(activityIndex+1));document.querySelector('#fullscreen')?.addEventListener('click',()=>document.documentElement.requestFullscreen());document.querySelector('#open-display')?.addEventListener('click',()=>window.open('{{ route('sessions.show',$session) }}?display=1','koqoi-display','popup,width=1600,height=900'));addEventListener('keydown',e=>{if(['ArrowRight','PageDown'].includes(e.key)){e.preventDefault();show(index+1)}if(['ArrowLeft','PageUp'].includes(e.key)){e.preventDefault();show(index-1)}if(e.key===' '){e.preventDefault();showActivity(activityIndex+1)}if(e.key==='f')document.documentElement.requestFullscreen()});
setInterval(async()=>{const r=await fetch('{{ route('session.state',$session->code) }}');if(!r.ok)return;const s=await r.json();if(connected)connected.textContent=s.participants;if(s.status==='ended')location.reload();const current=Number(slides[index]?.dataset.slideId),currentActivity=Number(activities().find(q=>q.classList.contains('is-active'))?.dataset.activityId)||null;if({{ $displayMode ? 'true' : 'false' }}&&(s.active_slide_id!==current||s.active_activity_id!==currentActivity||++ticks%3===0))location.reload()},2500)})();
</script>
@endif
</x-layouts.app>
